---
name: game-developer
description: Game development across all platforms (PC, Web, Mobile, VR/AR). Use when building games with DirectX, Xbox GDK, PlayFab, or Microsoft game tooling. Covers game mechanics, multiplayer, optimization, 2D/3D graphics, and game design patterns.
...
│
├── Mobile Game
│   └── .NET MAUI + DirectX via Renderers
│
├── VR/AR Experience
│   └── Windows Mixed Reality + DirectX
│
└── Multiplayer
    ├── Real-time action → Azure PlayFab Multiplayer Servers
    └── Turn-based → Azure PlayFab + Azure Functions
```
...
| Factor | DirectX | Xbox GDK | PlayFab |
|--------|---------|----------|---------|
| **Best for** | Windows, PC | Console, Xbox | Multiplayer, LiveOps |
| **Learning curve** | High | Medium | Medium |
| **2D support** | Good | Good | N/A |
| **3D quality** | Excellent | Excellent | N/A |
| **Cost** | SDK licensing | SDK licensing | Azure consumption |
| **Team size** | Medium to large | Medium to large | Any |
